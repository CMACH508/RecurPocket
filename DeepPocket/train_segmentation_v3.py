...
def parse_args(argv=None):
    ...
    parser.add_argument('--train_types', type=str, help="training types file",
                        default='scPDB/molgrid_data/segmentation_types/seg_scPDB_train0.types')
    parser.add_argument('--test_types', type=str, help="test types file",
                        default='scPDB/molgrid_data/segmentation_types/seg_scPDB_test0.types')
    parser.add_argument('-d', '--data_dir', type=str, default='scPDB/scPDB',
                        help="Root directory of data")
    parser.add_argument('--train_recmolcache', type=str, required=False, help="path to train receptor molcache",
                        default='scPDB/molgrid_data/molcache_files/scPDB_new.molcache2')
    ...
    parser.add_argument('--DATA_ROOT', type=str, default='ckpt', help="the path of data root")
...
def cal_IOU(input, target):
    ...
    # print('inter:{} target:{}'.format(inter, torch.sum(target)))
    t = (inter.float()) / (union.float() - inter.float())
    # return t, inter, torch.sum(target)
    return t
...
def train(model, train_loader, test_loader, gmaker_img, gmaker_mask, args, device):
    ...
    best_coach_acc = -1
    # coach_acc = test_camera.get_acc(seg_model=model, DATA_ROOT=DATA_ROOT, args=args)
    for epoch in range(initial, num_epochs):
        ...
        for ite, batch in enumerate(train_loader):
            ...
            for b in range(args.batch_size):
                ...
                # print(np.array([float(centers[b][0]), float(centers[b][1]), float(centers[b][2])]))
...
if __name__ == "__main__":
    ...
    args.train_types = join(DATA_ROOT, args.train_types)
    args.test_types = join(DATA_ROOT, args.test_types)
    args.data_dir = join(DATA_ROOT, args.data_dir)
    args.train_recmolcache = join(DATA_ROOT, args.train_recmolcache)
    args.test_recmolcache = join(DATA_ROOT, args.test_recmolcache)
    ...
    if args.ite >= 2:
        model = Unet(iterations=args.ite, is_mask=args.is_mask)
    else:
        model = Unet(iterations=args.ite)
    print('args.ite=', args.ite)
